...
def create_center_filter_complex(row,filter_length,last_fb,max_length,class_function):
...
#	filters = [filter_class.get_filters(t) for t in Ts]
        filter_bank,_=theano.scan(fn = lambda row,filter_length,last_fb,max_length: create_center_filter_complex(row,filter_length,last_fb,max_length,filter_class.get_filters),sequences=[theano.tensor.arange(J*Q+1,dtype='int32'),Ts_t],non_sequences=[Ts_t[-1]],n_steps=J*Q+1,outputs_info=theano.tensor.zeros(((J*Q+1)*2,Ts[-1]),dtype='float32'))
